...
def format_pattern_number(number):
    """패턴 넘버를 문자열로 변환하고 한 자리 숫자 앞에 0을 추가합니다."""
...
for group in ['groupA', 'groupB']:
    for pattern in data['patterns'][group]:
        ...
        # 패턴 넘버를 문자열로 변환하고 한 자리 숫자 앞에 0 추가
        if 'pattern_number' in pattern:
            pattern['pattern_number'] = format_pattern_number(pattern['pattern_number'])
