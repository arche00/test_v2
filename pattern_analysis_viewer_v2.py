...
def calculate_round4_pick(validation_results):
    """
    라운드 4의 Pick(실제문자) 계산 로직:
    - round4_validation(가정값)이 'W'면 Pick은 예측문자(P4)와 같음
    - round4_validation(가정값)이 'L'면 Pick은 예측문자(P4)의 반대값
    - game_result가 'P'이든 'F'이든 동일하게 적용
    """
    if len(validation_results) < 3:
        return None
    
    round3_result = next((r for r in validation_results if r['라운드'] == 3), None)
    round2_result = next((r for r in validation_results if r['라운드'] == 2), None)
    round4_result = next((r for r in validation_results if r['라운드'] == 4), None)
    
    if not round3_result or not round2_result or not round4_result:
        return None
    
    # round4_validation 계산 (game_result에 따라)
    if round3_result.get('game_result') == 'P':
        round4_validation = 'L' if round2_result.get('validation_result') == 'W' else 'W'
    else:
        round4_validation = round2_result.get('validation_result')
    
    p4 = round4_result.get('예측문자')
    if not p4:
        return None
    
    # Pick 계산 (역산)
    if round4_validation == 'W':
        return p4
    else:
        return 'L' if p4 == 'W' else 'W'

def display_round4_pick(validation_results):
    """라운드 4 Pick을 표시하는 함수"""
    import streamlit as st
    
    # 라운드 4 Pick 계산
    round4_pick = calculate_round4_pick(validation_results)
    
    # 결과 표시
    if round4_pick:
        st.write(f"라운드 4 Pick: {round4_pick}")
    
    return round4_pick

def display_validation_results_with_pick(validation_results, title):
    """검증 결과와 라운드 4 Pick을 표시하는 함수"""
    import streamlit as st
    ...
    if validation_results:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "실제문자": st.column_config.TextColumn("실제문자"),
                "결과": st.column_config.TextColumn("결과"),
                "빈도정보": st.column_config.TextColumn("빈도정보")
            }
        )
        
        # 라운드 4 Pick 표시
        display_round4_pick(validation_results)
...
def main():
    ...
    if actual_pattern:
        ...
        # 통합 결과 리스트를 main에서 직접 생성하여 Pick 계산
        odd_rounds_3char = [r for r in validation_results_3char if r['라운드'] % 2 == 1]
        even_rounds_4char = [r for r in validation_results_4char if r['라운드'] % 2 == 0]
        combined_results = odd_rounds_3char + even_rounds_4char
        # 라운드 4 Pick 및 Result 표시
        round4_result = next((r for r in combined_results if r['라운드'] == 4), None)
        pick = calculate_round4_pick(combined_results)
        actual = round4_result['실제문자'] if round4_result else ''
        if actual == '':
            result = ''
        else:
            result = 'W' if pick == actual else 'L'
        st.write(f"라운드 4 Pick: {pick}   Result: {result}")
        
        # 3자리와 4자리 결과를 수평으로 배치
        col1, col2 = st.columns(2)
        with col1:
            display_validation_results(validation_results_3char, "3자리 패턴 검증 결과")
            display_round4_pick(validation_results_3char)
        with col2:
            display_validation_results(validation_results_4char, "4자리 패턴 검증 결과")
            display_round4_pick(validation_results_4char)
