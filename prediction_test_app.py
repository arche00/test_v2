...
def get_pattern_sequence_type(zone):
    """Get pattern sequence type from zone data"""
    try:
...
def get_pattern_results(zone):
    ...
    try:
        ...
        # Get sequence type
        sequence_type = get_pattern_sequence_type(zone)
        
        # Get predictions
        prediction1_2, found1_2 = get_pattern_prediction(pattern1_2_combined, sequence_type)
        prediction1_2_3, found1_2_3 = get_pattern123_prediction(pattern1_2_3_combined, sequence_type)
        ...
        return pattern1_2, pattern1_2_3, prediction1_2, prediction1_2_3, comparison1_2, comparison1_2_3, sequence_type, pattern1_2_combined, pattern1_2_3_combined
    except Exception as e:
        return '', '', '', '', '', '', '', '', ''
...
def save_pattern_analysis(zones, session_id):
    ...
    try:
        ...
        for idx, zone in enumerate(zones, 1):
            ...
            # Get pattern results
            pattern1_2, pattern1_2_3, prediction1_2, prediction1_2_3, comparison1_2, comparison1_2_3, sequence_type, pattern1_2_combined, pattern1_2_3_combined = get_pattern_results(zone)
    ...
    with right_col:
                                ...
                                if all_prediction_results:
                                    ...
                                    cursor.execute('''
                                        CREATE TABLE IF NOT EXISTS session_prediction_results (
                                            ...
                                            created_at TIMESTAMP DEFAULT (datetime('now', 'localtime', '+9 hours'))
                                        )
                                    ''')
        ...
        if st.session_state.show_grid and st.session_state.converted_grid is not None:
            ...
            if zones:
                ...
                # Display individual group results
                for zone in sorted_zones:
                    group_range = f"{zone['start_x'] + 1}-{zone['end_x'] + 1}"
                    st.markdown(f"#### Group {group_range}")
                    pattern1_2, pattern1_2_3, prediction1_2, prediction1_2_3, comparison1_2, comparison1_2_3, sequence_type, pattern1_2_combined, pattern1_2_3_combined = get_pattern_results(zone)
