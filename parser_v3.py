...
def main():
    # Set full page width
    # st.set_page_config(layout="wide")  # 이 줄 제거
    
    # 상단 여백 최소화 및 expander 내부 모든 여백 완전 제거
    st.markdown("""
        <style>
        .stApp {margin-top: -2.5rem;}
        /* expander 내부 모든 컨테이너와 요소의 여백/패딩 완전 제거 */
        ...
        .bp-btn-row {display: flex; gap: 2.5rem; margin-bottom: 0.1rem; justify-content: flex-start;}
        .bp-btn {
            font-size: 2.1rem; font-weight: bold; padding: 1.1rem 3.5rem; border-radius: 1.2rem;
            border: 3px solid #888; background: #f7f7f7; cursor: pointer; transition: background 0.2s, border 0.2s;
            margin-bottom: 0.1rem;
        }
        .bp-btn.selected {background: #e0e7ff; border: 3px solid #3b82f6; color: #1e40af;}
        .bp-btn:hover {background: #f0f2f6;}
        </style>
    """, unsafe_allow_html=True)
    ...
    if 'pattern1_number' not in st.session_state:
        st.session_state.pattern1_number = ''
    if 'pattern2_number' not in st.session_state:
        st.session_state.pattern2_number = ''
    ...
    with left_col:
        ...
        if st.session_state.show_grid and st.session_state.grid is not None:
            ...
            # Minimize spacing for expander and form elements
            st.markdown("""
                <style>
                div[data-testid="stExpander"] div[role="button"] {margin-bottom: 0.1rem;}
                div[data-testid="stExpander"] {margin-bottom: 0.1rem;}
                div[data-testid="stExpander"] label, div[data-testid="stExpander"] .stSelectbox, div[data-testid="stExpander"] .stButton, div[data-testid="stExpander"] .stInfo, div[data-testid="stExpander"] .stAlert {margin: 0 !important; padding: 0.1rem 0 !important;}
                div[data-testid="stExpander"] .stSelectbox > div {margin: 0 !important; padding: 0.1rem 0 !important;}
                div[data-testid="stExpander"] .stButton button {margin: 0 !important; padding: 0.1rem 0.5rem !important; min-height: 1.5rem;}
                </style>
            """, unsafe_allow_html=True)
            ...
            # Process zones and display pattern analysis with edited grid
    ...
    # Right column: Number Prediction UI
    with right_col:
        ...
        with col1:
            if 'p1_input_key' not in st.session_state:
                st.session_state.p1_input_key = 0
            st.markdown("#### Pattern1 → Result1 예측")
            pattern1_number = st.text_input(
                "pattern1_number 입력",
                value=st.session_state.get('pattern1_number', ''),
                key=f"p1_input_{st.session_state.p1_input_key}"
            )
            predict1_clicked = st.button("Pattern1 예측", key="p1_btn")
            p1_reset_clicked = st.button("초기화", key="p1_reset_btn")
            if predict1_clicked:
                st.session_state.pattern1_number = pattern1_number
                st.session_state.p1_input_key += 1
                with st.spinner("예측 중..."):
                    predictions = predict_result1(pattern1_number)
                    if predictions:
                        logs = []
                        # --- LOGGING/DEBUG OUTPUT ONLY ON BUTTON CLICK ---
                        pattern_data = load_pattern_data()
                        if not pattern_data:
                            filtered_predictions = predictions[:3]
                        else:
                            search_prefix = str(pattern1_number)[:2]
                            logs.append(f"검색 접두사: {search_prefix}")
                            initial_patterns = search_patterns(pattern_data, search_prefix)
                            logs.append(f"초기 패턴 검색 결과 수: {len(initial_patterns)}")
                            target_sequences = []
                            for pattern in initial_patterns:
                                sequence = pattern['sequence']
                                if len(sequence) >= 6:
                                    target_seq = ''.join(sequence[3:6]).lower()
                                    target_sequences.append(target_seq)
                                    logs.append(f"추출된 시퀀스: {target_seq} (패턴 번호: {pattern['pattern_number']})")
                            related_patterns = []
                            for seq in target_sequences:
                                for group_name in ['groupA', 'groupB']:
                                    patterns = pattern_data['patterns'][group_name]
                            ...
                            logs.append(f"시퀀스 '{seq}'로 시작하는 패턴 수: {len(related_patterns)}")
                            related_patterns_dict = {}
                            for pattern in related_patterns:
                                if pattern['pattern_number'] != 'N/A':
                                    related_patterns_dict[pattern['pattern_number']] = {
                            ...
                            logs.append(f"관련 패턴 번호 수: {len(related_patterns_dict)}")
                            logs.append(f"관련 패턴 번호: {sorted(list(related_patterns_dict.keys()))}")
                            filtered_predictions = []
                            for pred in predictions:
                                pred_number = str(pred['number'])
                                if pred_number in related_patterns_dict:
                                    ...
                                    seq_str = ' '.join(pred_info['sequence'])
                                    logs.append(f"필터링된 예측: {pred_number} (확률: {pred['probability']:.2%}, 그룹: {pred_info['group']}, 값: {pred_info['group_value']}) 시퀀스: {seq_str}")
                            if not filtered_predictions:
                                logs.append("필터링된 예측이 없습니다. 상위 3개 예측을 반환합니다.")
                                filtered_predictions = predictions[:3]
                            else:
                                filtered_predictions.sort(key=lambda x: x['probability'], reverse=True)
                                logs.append(f"최종 필터링된 예측 수: {len(filtered_predictions)}")
                            filtered_predictions = filtered_predictions[:3]
                        st.session_state.result1 = filtered_predictions
                        st.session_state.result1_logs = logs
            if p1_reset_clicked:
                st.session_state.result1 = None
                st.session_state.result1_logs = None
            if 'result1' in st.session_state and st.session_state.result1:
                if 'result1_logs' in st.session_state:
                    for log in st.session_state.result1_logs:
                        st.write(log)
                st.success("이전 예측 결과:")
        ...
        with col2:
            if 'p2_input_key' not in st.session_state:
                st.session_state.p2_input_key = 0
            st.markdown("#### Pattern2 → Result2 예측")
            pattern2_number = st.text_input(
                "pattern2_number 입력",
                value=st.session_state.get('pattern2_number', ''),
                key=f"p2_input_{st.session_state.p2_input_key}"
            )
            predict2_clicked = st.button("Pattern2 예측", key="p2_btn")
            p2_reset_clicked = st.button("초기화", key="p2_reset_btn")
            if predict2_clicked:
                st.session_state.pattern2_number = pattern2_number
                st.session_state.p2_input_key += 1
                with st.spinner("예측 중..."):
                    ...
                    if predictions:
                        ...
                        if not pattern_data:
                            ...
                            pattern2_str = str(pattern2_number)
                            if len(pattern2_str) < 4:
                                logs.append("pattern2_number가 너무 짧습니다. (최소 4자리 필요)")
                                filtered_predictions = predictions[:3]
                            else:
                                search_prefix = pattern2_str[2:4]
                                logs.append(f"Pattern2 검색 접두사(3,4번째): {search_prefix}")
                                initial_patterns = search_patterns(pattern_data, search_prefix)
                                logs.append(f"초기 패턴 검색 결과 수: {len(initial_patterns)}")
                                target_sequences = []
                                for pattern in initial_patterns:
                    ...
                    st.session_state.result2_logs = logs
            if p2_reset_clicked:
                st.session_state.result2 = None
                st.session_state.result2_logs = None
            if 'result2' in st.session_state and st.session_state.result2:
                if 'result2_logs' in st.session_state:
                    for log in st.session_state.result2_logs:
                        st.write(log)
                st.success("이전 예측 결과:")
